...
class TestModuleInit:
    ...
    def test_imports_from_init(self):
        """Test that all exports are available from src.ioc."""
        import src.ioc as ioc

        # Bootstrap
        assert hasattr(ioc, 'initialize_ioc_app')
        assert hasattr(ioc, 'create_container')

        # Lifecycle
        assert hasattr(ioc, 'initialize_components')
        assert hasattr(ioc, 'shutdown_components')

        # DI
        assert hasattr(ioc, 'get_library')
        assert hasattr(ioc, 'get_config')
        assert hasattr(ioc, 'get_logger')

        # Config
        assert hasattr(ioc, 'Settings')
        assert hasattr(ioc, 'register_configuration')

        # Components
        assert hasattr(ioc, 'Component')
        assert hasattr(ioc, 'AppComponent')

    def test_all_list_complete(self):
        """Test that __all__ list matches actual exports."""
        import src.ioc as ioc

        for name in ioc.__all__:
            assert hasattr(ioc, name), f"Missing export: {name}"
